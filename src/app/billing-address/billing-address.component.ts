...
export class BillingAddressComponent implements OnInit {
        ...
        .subscribe(
          UpdateResponse => {
            ...
            if (this.UpdateResponse[0]["status"]) {
              ...
              // updating current billing address
              sessionStorage.setItem('currentBillingAddress', JSON.stringify(UserAddressInfo));
  ...
  removeCartItem(userId, cartList) {
    const removeItem = (item, pos) => {
      this.shoppingCartService.RemoveCartService(userId, item.product_id)
        .subscribe(
          cart => {
            console.log("Cart:" + JSON.stringify(cart));
            if (cart[0]["status"] == "true") {
              if (pos === cartList.length - 1) {
                alert('Order placed successfully');
                this.cartDataService.updateCart(cart);
